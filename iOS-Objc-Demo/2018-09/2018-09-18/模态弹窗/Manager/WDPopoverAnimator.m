...
@interface WDPopoverAnimator ()
{
    BOOL     _isPresented;
    CGSize   _presentedSize;
}
@property(nonatomic, strong) WDPresentationController  *presentationController;
@property(nonatomic, assign )WDPopoverType              popoverType;
@property(nonatomic, assign) CGFloat                    presentedHeight;
@property(nonatomic, copy)  WDCompleteHandle           completeHandle;

@end

@implementation WDPopoverAnimator

+ (instancetype)popoverAnimatorWithStyle:(WDPopoverType )popoverType completeHandle:(WDCompleteHandle)completeHandle{
    WDPopoverAnimator *popoverAnimator = [[WDPopoverAnimator alloc] init];
...
- (nullable UIPresentationController *)presentationControllerForPresentedViewController:(UIViewController *)presented presentingViewController:(nullable UIViewController *)presenting sourceViewController:(UIViewController *)source{
    WDPresentationController *presentation = [[WDPresentationController alloc] initWithPresentedViewController:presented presentingViewController:presenting];
...
- (void)animationForPresentedView:(nonnull id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    WeakSelf(weakSelf)
    if (self.popoverType == WDPopoverTypeAlert) {
