...
@implementation DNSortContactsSetting

static DNSortContactsSetting * singleton = nil ;

+(DNSortContactsSetting*)share {
    
    if (singleton == nil) {
        singleton = [[DNSortContactsSetting alloc] init];
...
@implementation DNSortContactsModel
...
@implementation DNSortContactsTool
...
+(void)sortAndGroup:(NSArray*)objectArray
    ...
    if (key == nil) {
        if (![obj isKindOfClass:NSString.class]) {
            [DNSortContactsTool logMsg:@"数组内元素不是字符串类型,如果是对象类型，请传key"];
    ...
    if (!containKey) {
        [DNSortContactsTool logMsg:@"数组内元素未包含指定属性"];
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [objectArray enumerateObjectsWithOptions:NSEnumerationConcurrent usingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            DNSortContactsModel *model = [self getModelWithObj:obj key:key];
            if (model) {
                //对 数组的插入操作 上锁
        ...
        [DNSortContactsTool logMsg:@""];
        [DNSortContactsTool logMsg:[NSString stringWithFormat:@"转拼音用时：\t %f s", (state1-start)]];
        ...
        [DNSortContactsTool logMsg:[NSString stringWithFormat:@"排序用时：\t %f s", (state2-state1)]];
        ...
        for (DNSortContactsModel* object in sortModelArray) {
        ...
        [DNSortContactsTool logMsg:[NSString stringWithFormat:@"分组用时：\t %f s", (state3-state2)]];
        [DNSortContactsTool logMsg:[NSString stringWithFormat:@"BMChineseSort 排序总计用时：\t %f s", (state3-start)]];
...
+(DNSortContactsModel *)getModelWithObj:(id)obj key:(NSString*)key{
    DNSortContactsModel *model = [[DNSortContactsModel alloc]init];
    model.object = obj;
    if (!key) {
        model.string = obj;
    }else{
    ...
    if(model.string == nil || model.string.length == 0){
        model.string = DNSortContactsSetting.share.specialCharSectionTitle;
    }else{
        //过滤 ignoreModelWithPrefix
        NSString *prefix = [model.string substringToIndex:1];
        if (![DNSortContactsSetting.share.ignoreModelWithPrefix containsString:prefix]) {
            //是否将字母与汉字拼音一起排序？？？？ （暂不考虑）
            //获取拼音首字母
...
+(void)logMsg:(NSString*)msg{
    if (DNSortContactsSetting.share.logEable == YES) {
        NSLog(@"%@", msg);
    }
}
...
+ (NSString *)getFirstLetter:(NSString *)chinese{
    ...
    //吧多音字先替换
    [DNSortContactsSetting.share.polyphoneMapping enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        newChinese = [newChinese stringByReplacingOccurrencesOfString:key withString:obj];
    }];
    
    NSMutableString *result = [NSMutableString string];
    
    if (DNSortContactsSetting.share.sortMode == 1) {
        //此处 对整个字符串转中文 而不单个字一次转 因为CFStringTransform太耗时 而这个时间又与字个数关系不大，所以尽量减少调用次数 以减少时间
        NSArray *wordArr = [[DNSortContactsTool transformChinese:newChinese] componentsSeparatedByString:@" "];
    ...
    if ([upperCaseStr characterAtIndex:0] >= 'A' && [upperCaseStr characterAtIndex:0] <= 'Z') {
        ...
        return DNSortContactsSetting.share.specialCharSectionTitle;
    }
}
